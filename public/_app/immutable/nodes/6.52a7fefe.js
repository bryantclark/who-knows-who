import { d as Ze } from '../chunks/environment.9aa685ef.js';
import {
	S as $e,
	i as et,
	s as tt,
	k as n,
	a as i,
	q as s,
	Q as st,
	l,
	h as t,
	c as d,
	m as r,
	r as a,
	n as o,
	F as e,
	b as Ke,
	G as Ie
} from '../chunks/index.1eb2484a.js';
const at = Ze,
	nt = !0,
	ct = Object.freeze(
		Object.defineProperty({ __proto__: null, csr: at, prerender: nt }, Symbol.toStringTag, {
			value: 'Module'
		})
	);
function lt(Xe) {
	let _,
		G,
		c,
		F,
		U,
		Y,
		E,
		Q,
		w,
		R,
		B,
		K,
		p,
		q,
		X,
		Z,
		P,
		$,
		ee,
		b,
		te,
		se,
		A,
		ae,
		ne,
		N,
		le,
		re,
		u,
		oe,
		T,
		ce,
		ie,
		k,
		de,
		pe,
		H,
		he,
		ve,
		ue,
		h,
		I,
		me,
		fe,
		D,
		_e,
		ge,
		O,
		xe,
		Ee,
		W,
		Se,
		ye,
		z,
		we,
		qe,
		S,
		Pe,
		V,
		be,
		Ae,
		Ne,
		y,
		Te,
		j,
		ke,
		He;
	return {
		c() {
			((_ = n('meta')),
				(G = i()),
				(c = n('div')),
				(F = n('h1')),
				(U = s('How to play Sverdle')),
				(Y = i()),
				(E = n('p')),
				(Q = s('Sverdle is a clone of ')),
				(w = n('a')),
				(R = s('Wordle')),
				(B = s(`, the
		word guessing game. To play, enter a five-letter English word. For example:`)),
				(K = i()),
				(p = n('div')),
				(q = n('span')),
				(X = s('r')),
				(Z = i()),
				(P = n('span')),
				($ = s('i')),
				(ee = i()),
				(b = n('span')),
				(te = s('t')),
				(se = i()),
				(A = n('span')),
				(ae = s('z')),
				(ne = i()),
				(N = n('span')),
				(le = s('y')),
				(re = i()),
				(u = n('p')),
				(oe = s('The ')),
				(T = n('span')),
				(ce = s('y')),
				(ie = s(' is in the right place. ')),
				(k = n('span')),
				(de = s('r')),
				(pe = s(` and
		`)),
				(H = n('span')),
				(he = s('t')),
				(ve = s(`
		are the right letters, but in the wrong place. The other letters are wrong, and can be discarded.
		Let's make another guess:`)),
				(ue = i()),
				(h = n('div')),
				(I = n('span')),
				(me = s('p')),
				(fe = i()),
				(D = n('span')),
				(_e = s('a')),
				(ge = i()),
				(O = n('span')),
				(xe = s('r')),
				(Ee = i()),
				(W = n('span')),
				(Se = s('t')),
				(ye = i()),
				(z = n('span')),
				(we = s('y')),
				(qe = i()),
				(S = n('p')),
				(Pe = s('This time we guessed right! You have ')),
				(V = n('strong')),
				(be = s('six')),
				(Ae = s(' guesses to get the word.')),
				(Ne = i()),
				(y = n('p')),
				(Te =
					s(`Unlike the original Wordle, Sverdle runs on the server instead of in the browser, making it
		impossible to cheat. It uses `)),
				(j = n('code')),
				(ke = s('<form>')),
				(He = s(` and cookies to submit data, meaning you can
		even play with JavaScript disabled!`)),
				this.h());
		},
		l(g) {
			const C = st('svelte-1fqqyy5', document.head);
			((_ = l(C, 'META', { name: !0, content: !0 })),
				C.forEach(t),
				(G = d(g)),
				(c = l(g, 'DIV', { class: !0 })));
			var v = r(c);
			F = l(v, 'H1', {});
			var De = r(F);
			((U = a(De, 'How to play Sverdle')), De.forEach(t), (Y = d(v)), (E = l(v, 'P', {})));
			var J = r(E);
			((Q = a(J, 'Sverdle is a clone of ')), (w = l(J, 'A', { href: !0 })));
			var Oe = r(w);
			((R = a(Oe, 'Wordle')),
				Oe.forEach(t),
				(B = a(
					J,
					`, the
		word guessing game. To play, enter a five-letter English word. For example:`
				)),
				J.forEach(t),
				(K = d(v)),
				(p = l(v, 'DIV', { class: !0 })));
			var m = r(p);
			q = l(m, 'SPAN', { class: !0 });
			var We = r(q);
			((X = a(We, 'r')), We.forEach(t), (Z = d(m)), (P = l(m, 'SPAN', { class: !0 })));
			var ze = r(P);
			(($ = a(ze, 'i')), ze.forEach(t), (ee = d(m)), (b = l(m, 'SPAN', { class: !0 })));
			var Fe = r(b);
			((te = a(Fe, 't')), Fe.forEach(t), (se = d(m)), (A = l(m, 'SPAN', { class: !0 })));
			var Ve = r(A);
			((ae = a(Ve, 'z')), Ve.forEach(t), (ne = d(m)), (N = l(m, 'SPAN', { class: !0 })));
			var je = r(N);
			((le = a(je, 'y')), je.forEach(t), m.forEach(t), (re = d(v)), (u = l(v, 'P', { class: !0 })));
			var x = r(u);
			((oe = a(x, 'The ')), (T = l(x, 'SPAN', { class: !0 })));
			var Ce = r(T);
			((ce = a(Ce, 'y')),
				Ce.forEach(t),
				(ie = a(x, ' is in the right place. ')),
				(k = l(x, 'SPAN', { class: !0 })));
			var Ge = r(k);
			((de = a(Ge, 'r')),
				Ge.forEach(t),
				(pe = a(
					x,
					` and
		`
				)),
				(H = l(x, 'SPAN', { class: !0 })));
			var Je = r(H);
			((he = a(Je, 't')),
				Je.forEach(t),
				(ve = a(
					x,
					`
		are the right letters, but in the wrong place. The other letters are wrong, and can be discarded.
		Let's make another guess:`
				)),
				x.forEach(t),
				(ue = d(v)),
				(h = l(v, 'DIV', { class: !0 })));
			var f = r(h);
			I = l(f, 'SPAN', { class: !0 });
			var Le = r(I);
			((me = a(Le, 'p')), Le.forEach(t), (fe = d(f)), (D = l(f, 'SPAN', { class: !0 })));
			var Me = r(D);
			((_e = a(Me, 'a')), Me.forEach(t), (ge = d(f)), (O = l(f, 'SPAN', { class: !0 })));
			var Ue = r(O);
			((xe = a(Ue, 'r')), Ue.forEach(t), (Ee = d(f)), (W = l(f, 'SPAN', { class: !0 })));
			var Ye = r(W);
			((Se = a(Ye, 't')), Ye.forEach(t), (ye = d(f)), (z = l(f, 'SPAN', { class: !0 })));
			var Qe = r(z);
			((we = a(Qe, 'y')), Qe.forEach(t), f.forEach(t), (qe = d(v)), (S = l(v, 'P', {})));
			var L = r(S);
			((Pe = a(L, 'This time we guessed right! You have ')), (V = l(L, 'STRONG', {})));
			var Re = r(V);
			((be = a(Re, 'six')),
				Re.forEach(t),
				(Ae = a(L, ' guesses to get the word.')),
				L.forEach(t),
				(Ne = d(v)),
				(y = l(v, 'P', {})));
			var M = r(y);
			((Te = a(
				M,
				`Unlike the original Wordle, Sverdle runs on the server instead of in the browser, making it
		impossible to cheat. It uses `
			)),
				(j = l(M, 'CODE', {})));
			var Be = r(j);
			((ke = a(Be, '<form>')),
				Be.forEach(t),
				(He = a(
					M,
					` and cookies to submit data, meaning you can
		even play with JavaScript disabled!`
				)),
				M.forEach(t),
				v.forEach(t),
				this.h());
		},
		h() {
			((document.title = 'How to play Sverdle'),
				o(_, 'name', 'description'),
				o(_, 'content', 'How to play Sverdle'),
				o(w, 'href', 'https://www.nytimes.com/games/wordle/index.html'),
				o(q, 'class', 'close svelte-1x5nq1n'),
				o(P, 'class', 'missing svelte-1x5nq1n'),
				o(b, 'class', 'close svelte-1x5nq1n'),
				o(A, 'class', 'missing svelte-1x5nq1n'),
				o(N, 'class', 'exact svelte-1x5nq1n'),
				o(p, 'class', 'example svelte-1x5nq1n'),
				o(T, 'class', 'exact svelte-1x5nq1n'),
				o(k, 'class', 'close svelte-1x5nq1n'),
				o(H, 'class', 'close svelte-1x5nq1n'),
				o(u, 'class', 'svelte-1x5nq1n'),
				o(I, 'class', 'exact svelte-1x5nq1n'),
				o(D, 'class', 'exact svelte-1x5nq1n'),
				o(O, 'class', 'exact svelte-1x5nq1n'),
				o(W, 'class', 'exact svelte-1x5nq1n'),
				o(z, 'class', 'exact svelte-1x5nq1n'),
				o(h, 'class', 'example svelte-1x5nq1n'),
				o(c, 'class', 'text-column'));
		},
		m(g, C) {
			(e(document.head, _),
				Ke(g, G, C),
				Ke(g, c, C),
				e(c, F),
				e(F, U),
				e(c, Y),
				e(c, E),
				e(E, Q),
				e(E, w),
				e(w, R),
				e(E, B),
				e(c, K),
				e(c, p),
				e(p, q),
				e(q, X),
				e(p, Z),
				e(p, P),
				e(P, $),
				e(p, ee),
				e(p, b),
				e(b, te),
				e(p, se),
				e(p, A),
				e(A, ae),
				e(p, ne),
				e(p, N),
				e(N, le),
				e(c, re),
				e(c, u),
				e(u, oe),
				e(u, T),
				e(T, ce),
				e(u, ie),
				e(u, k),
				e(k, de),
				e(u, pe),
				e(u, H),
				e(H, he),
				e(u, ve),
				e(c, ue),
				e(c, h),
				e(h, I),
				e(I, me),
				e(h, fe),
				e(h, D),
				e(D, _e),
				e(h, ge),
				e(h, O),
				e(O, xe),
				e(h, Ee),
				e(h, W),
				e(W, Se),
				e(h, ye),
				e(h, z),
				e(z, we),
				e(c, qe),
				e(c, S),
				e(S, Pe),
				e(S, V),
				e(V, be),
				e(S, Ae),
				e(c, Ne),
				e(c, y),
				e(y, Te),
				e(y, j),
				e(j, ke),
				e(y, He));
		},
		p: Ie,
		i: Ie,
		o: Ie,
		d(g) {
			(t(_), g && t(G), g && t(c));
		}
	};
}
class it extends $e {
	constructor(_) {
		(super(), et(this, _, null, lt, tt, {}));
	}
}
export { it as component, ct as universal };
