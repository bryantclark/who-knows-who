...
const Mt = typeof window < 'u' ? window : typeof globalThis < 'u' ? globalThis : global;
let T = !1;
function rt() {
	T = !0;
}
...
function lt(t) {
	...
	for (let c = 0; c < e.length; c++) {
		const l = e[c].claim_order,
			f = (r > 0 && e[n[r]].claim_order <= l ? r + 1 : ct(1, r, (d) => e[n[d]].claim_order, l)) - 1;
		i[c] = n[f] + 1;
		const _ = f + 1;
...
function Tt(t, e, n) {
	T && !n ? ot(t, e) : (e.parentNode !== t || e.nextSibling != n) && t.insertBefore(e, n || null);
}
function ut(t) {
...
function Z() {
	...
	do {
		...
		for (v(null), b.length = 0, x = 0; G.length; ) G.pop()();
...
function Yt(t, e, n, i, r, u, s, o, c, l, f, _) {
	let d = t.length,
		m = u.length,
		h = d;
	const j = {};
	...
	for (h = m; h--; ) {
		const a = _(r, u, h),
			p = n(a);
		let y = s.get(p);
		(y ? i && z.push(() => y.p(a, e)) : ((y = l(p, a)), y.c()),
			C.set(p, (S[h] = y)),
			p in j && D.set(p, Math.abs(h - j[p])));
	}
	const F = new Set(),
		R = new Set();
	function L(a) {
		(tt(a, 1), a.m(o, f), s.set(a.key, a), (f = a.first), m--);
	}
	for (; d && m; ) {
		const a = S[m - 1],
			p = t[d - 1],
...
function yt(t, e, n, i) {
	...
	(r && r.m(e, n),
		i ||
			B(() => {
				const s = t.$$.on_mount.map(J).filter(H);
				(t.$$.on_destroy ? t.$$.on_destroy.push(...s) : $(s), (t.$$.on_mount = []));
			}),
		u.forEach(B));
}
function gt(t, e) {
...
function xt(t, e) {
	(t.$$.dirty[0] === -1 && (b.push(t), Y(), t.$$.dirty.fill(0)),
		(t.$$.dirty[(e / 31) | 0] |= 1 << e % 31));
}
function ee(t, e, n, i, r, u, s, o = [-1]) {
	const c = N;
	v(t);
	const l = (t.$$ = {
		fragment: null,
	...
	if (
		((l.ctx = n
			? n(t, e.props || {}, (_, d, ...m) => {
					...
					return (
						l.ctx &&
							r(l.ctx[_], (l.ctx[_] = h)) &&
							(!l.skip_bound && l.bound[_] && l.bound[_](h), f && xt(t, _)),
						d
					);
				})
			: []),
		...
		(e.intro && tt(t.$$.fragment), yt(t, e.target, e.anchor, e.customElement), st(), Z());
	}
	v(c);
}
class ne {
	$destroy() {
		(gt(this, 1), (this.$destroy = E));
	}
	$on(e, n) {
		if (!H(n)) return E;
