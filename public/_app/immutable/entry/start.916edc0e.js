import { o as me, t as _e } from '../chunks/index.fb970b68.js';
import {
	S as Ke,
	a as ze,
	I as M,
	...
	P as Me,
	h as Qe
} from '../chunks/singletons.05b12a8a.js';
...
window.fetch = (n, r) => (
	(n instanceof Request ? n.method : (r == null ? void 0 : r.method) || 'GET') !== 'GET' &&
		ae.delete(Ee(n)),
	Be(n, r)
);
const ae = new Map();
				...
				: new RegExp(
						`^${gt(n)
							.map((c) => {
								...
								if (h)
									return (
										r.push({ name: h[1], matcher: h[2], optional: !1, rest: !0, chained: !0 }),
										'(?:/(.*))?'
									);
								const w = /^\[\[(\w+)(?:=(\w+))?\]\]$/.exec(c);
								if (w)
									return (
										r.push({ name: w[1], matcher: w[2], optional: !0, rest: !1, chained: !0 }),
										'(?:/([^/]+))?'
									);
								if (!c) return;
								const S = c.split(/\[(.+?)\](?!\])/);
...
function xt(n, r) {
	...
	async function xe() {
		...
		const t = (W = {}),
			o = i && (await pe(i));
		if (t === W && o) {
			if (o.type === 'redirect') return re(new URL(o.location, e).href, {}, 1, t);
	...
	async function Ae(e) {
		return (
			(y = {
				id: e.id,
				promise: pe(e).then((i) => (i.type === 'loaded' && i.state.error && (y = null), i))
			}),
			y.promise
		);
	}
	...
	function Pe(e) {
		...
		(i && i.remove(),
			(H = e.props.page),
			(F = new n.root({ target: r, props: { ...e.props, stores: J, components: m }, hydrate: !0 })),
			ke(U));
		const t = {
			from: null,
			to: {
	...
	async function X({ url: e, params: i, branch: t, status: o, error: a, route: s, form: f }) {
		let g = 'never';
		for (const p of t) (p == null ? void 0 : p.slash) !== void 0 && (g = p.slash);
		...
		for (let p = 0; p < Math.max(t.length, u.branch.length); p += 1) {
			...
			((d == null ? void 0 : d.data) !== (P == null ? void 0 : P.data) && (L = !0),
				d && ((_ = { ..._, ...d.data }), L && (v.props[`data_${R}`] = _), (R += 1)));
		}
		return (
			(!u.url || e.href !== u.url.href || u.error !== a || (f !== void 0 && f !== H.form) || L) &&
				(v.props.page = {
					error: a,
					params: i,
	...
	async function ue({ loader: e, parent: i, url: t, params: o, route: a, server_data_node: s }) {
		...
		if ((_ = v.universal) != null && _.load) {
			...
			((f = (await v.universal.load.call(null, p)) ?? null), (f = f ? await vt(f, a.id) : null));
		}
		return {
			node: v,
			loader: e,
			server: s,
			universal: (L = v.universal) != null && L.load ? { type: 'data', data: f, uses: g } : null,
		...
		for (let E = 0; E < v.length; E += 1)
			if (v[E])
				try {
					...
					if (k instanceof Ve) return { type: 'redirect', location: k.location };
					let x = 500,
						I;
	...
	async function ie({ status: e, error: i, url: t, route: o }) {
		...
		const g = await ue({
			...
			v = { node: await h(), loader: h, universal: null, server: null, data: null };
		return await X({ url: t, params: a, branch: [g, v], status: e, error: i, route: null });
	}
	function Q(e, i) {
		if (ye(e, V)) return;
		const t = se(e);
		for (const o of l) {
			const a = o.exec(t);
			if (a) return { id: e.pathname + e.search, invalidating: i, route: o, params: nt(a), url: e };
		}
	}
	function se(e) {
	...
	async function ce({
		...
		if (((y = null), D)) {
			...
			const k = (await Promise.all(b.on_navigate.map((x) => x(R.navigation)))).filter(
				(x) => typeof x == 'function'
			);
			if (k.length > 0) {
				let x = function () {
	...
	function Xe() {
		...
		(w.addEventListener('mousedown', i), w.addEventListener('touchstart', i, { passive: !0 }));
		const t = new IntersectionObserver(
			(s) => {
				for (const f of s)
					f.isIntersecting && (oe(se(new URL(f.target.href))), t.unobserve(f.target));
			},
			{ threshold: 0 }
		);
		function o(s, f) {
		...
		apply_action: async (e) => {
			if (e.type === 'error') {
				...
				e.type === 'redirect'
					...
					: (F.$set({ form: null, page: { ...H, form: e.data, status: e.status } }),
						await _e(),
						F.$set({ form: e.data }),
						e.type === 'success' && Se());
		},
		_start_router: () => {
			...
			((history.scrollRestoration = 'manual'),
				addEventListener('beforeunload', (t) => {
					...
					o ? (t.preventDefault(), (t.returnValue = '')) : (history.scrollRestoration = 'auto');
				}),
				...
				w.addEventListener('click', (t) => {
					...
					ce({
						...
						redirect_count: 0,
						details: { state: {}, replaceState: v.replace_state ?? a.href === location.href },
				...
				w.addEventListener('submit', (t) => {
					...
					(p && A.append(p, (a == null ? void 0 : a.getAttribute('value')) ?? ''),
						...
						ce({
							...
							keepfocus: v ?? !1,
							redirect_count: 0,
							details: { state: {}, replaceState: R ?? f.href === location.href },
							nav_token: {},
							accepted: () => {},
							blocked: () => {},
							type: 'form'
				...
				addEventListener('popstate', async (t) => {
					...
					if (((W = {}), (o = t.state) != null && o[M])) {
						...
						if (
							((a = u.url) == null ? void 0 : a.href.split('#')[0]) === location.href.split('#')[0]
						) {
				...
				addEventListener('hashchange', () => {
					T && ((T = !1), history.replaceState({ ...history.state, [M]: ++U }, '', location.href));
				}));
				...
				const _ = t.map(async (A, p) => {
						...
						return (
							...
							ue({
								...
								parent: async () => {
									const P = {};
									for (let $ = 0; $ < p; $ += 1) Object.assign(P, (await _[$]).data);
									return P;
								},
...
async function He(n, r) {
	...
	if (
		((h = c.headers.get('content-type')) != null && h.includes('text/html') && (await B(n)), !c.ok)
	)
		...
		S.catch(() => {}),
		{
			navigation: {
				...
				to: l && {
					...
					route: { id: ((b = r == null ? void 0 : r.route) == null ? void 0 : b.id) ?? null },
					url: l
				},
				willUnload: !r,
				type: c,
