...
describe('calculateBatchScores', () => {
	...
	it('should handle empty strings by returning 0', async () => {
		const scores = await calculateBatchScores('Question', '', { user1: '' });
		expect(scores.user1).toBe(0);
	});

	it('should handle very long strings without crashing', async () => {
		const longText = 'a'.repeat(1000);
		const scores = await calculateBatchScores('Question', longText, { user1: longText });
		expect(scores.user1).toBe(1);
	});
});
